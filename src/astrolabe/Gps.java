...
static double ground_stations[]={
	//Latitude, longitude, hauteur par rapport � l'ellipso�de WGS84 (�poque 2005)
	...
	public Gps() {
		...
		//et cr�er les 32 satellites GPS
		...
		// WINDOWS file = Paths.get(".\\current.alm");
		//file = Paths.get("current.alm");
		
		//file = Paths.get();
		
		//try {
		//System.out.println(astro.monde.getToolkit().getClass().getResource("/astrolabe/current.alm").toString());
		//System.out.println(
	...
	//14/02/2019 : navcen.uscg.gov KO => utiliser : https://celestrak.com/GPS/almanac/Yuma/almanac.yuma.txt
		    	...
		    	try {
			    	if(!line.isEmpty())
			    		if(!line.startsWith("*")) {
		    				...
		    				//System.out.println("param�tre : "+stringTemp[0]+"  valeur : "+stringTemp[1].trim());
				    ...
				    System.out.println(aoobe.toString());
		    	}
		    }
		} catch (IOException x) {
		...
		//v�rification cr�ation satellites
		/*
		for (int i=0; i<this.satellitesGPS.length; i++) {
			if(this.satellitesGPS[i] != null)//certains satellites ne sont pas renseign�s dans l'almanach (exemple : satellite GPS n�4)
				System.out.println("Satellite n�"+this.satellitesGPS[i].id+" rate Of Right Ascen = "+this.satellitesGPS[i].rateOfRightAscen);
			else
				System.out.println("Satellite n�"+i+" non d�fini dans l'almanach");
		}
		*/
