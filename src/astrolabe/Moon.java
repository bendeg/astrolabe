...
public class Moon {
	...
	public void calculateMoon() {
		...
		// +
		// Transformation en coordonnées EQUATORIALES

		//mean longitudes of the Sun and the Moon (L')
		this.periodicTerms=new double[]{218.3164477, 481267.88123421, -0.0015786, 1/538841, -1/65194000};
		...
		this.periodicTerms=new double[]{297.8501921, 445267.1114034, -0.0018819, 1/545868, -1/113065000};
		...
		this.periodicTerms=new double[]{357.5291092, 35999.0502909, -0.0001536, 1/24490000};
		...
		this.periodicTerms=new double[]{134.9633964, 477198.8675055,	0.0087414, 1/69699, -1/14712000};
		...
		this.periodicTerms=new double[]{93.2720950, 483202.0175233, -0.0036539, -1/3526000, 1/863310000};
		...
		for(int i=0; i<=this.moonLongitudeRadiusPeriodicTerms.length-6; i+=6){
			...
			*/
			...
			if(Math.abs(this.moonLongitudeRadiusPeriodicTerms[i+1])==2)
				tempE=this.E*this.E;
			else if (Math.abs(this.moonLongitudeRadiusPeriodicTerms[i+1])==1)
				tempE=this.E;
			else tempE=1.0;
			...
			this.sumL+=this.moonLongitudeRadiusPeriodicTerms[i+4]
					*Math.sin(Math.toRadians(
							...
							+tempE*this.moonLongitudeRadiusPeriodicTerms[i+1]*this.moonMeanAnomalySun
		...
		for(int i=0; i<=this.moonLatitudePeriodicTerms.length-5; i+=5){
			...
			*/
			...
			if(Math.abs(this.moonLatitudePeriodicTerms[i+1])==2)
				tempE=this.E*this.E;
			else if (Math.abs(this.moonLatitudePeriodicTerms[i+1])==1)
				tempE=this.E;
			else tempE=1.0;
			...
			this.sumB+=this.moonLatitudePeriodicTerms[i+4]
					*Math.sin(Math.toRadians(
							...
							+tempE*this.moonLatitudePeriodicTerms[i+1]*this.moonMeanAnomalySun
		...
		//System.out.println("Lune, parallaxe équatoriale horizontale ="+this.angleDecimalToDMS(this.moonEquatorialHorizontalParallax));
